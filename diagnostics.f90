...
!      counts(i1,i2,1)=counts(i1,i2,1)+1
       ...
       subroutine diag_integrate_uvoir_bands(p,spect_bins,uvoir_fp,method)
       type (epacket) , intent (in) :: p
       real(8) , intent (in) :: spect_bins(:)
       integer , intent (in) :: method
       real(8) , intent (inout) :: uvoir_fp(Ntimes,8)
       ...
       do nb=1,size(uvoir_band_names)
         ...
         uvoir_fp(i1,nb)=uvoir_fp(i1,nb)+p%Etot/delta*phi
       enddo
      
       return
       end subroutine diag_integrate_uvoir_bands

       subroutine diag_summ_uvoir(uvoir_fp)
       real(8) , intent (inout) :: uvoir_fp(Ntimes,8)
       uvoir_f=uvoir_f+uvoir_fp
       end subroutine diag_summ_uvoir
