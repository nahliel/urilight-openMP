      ...
      real(8) :: Mej,Vej,Rej,Vejmax,fac,vejfacmax
      integer :: nmix,i1,i2
...
!     10 - read w7
      ...
      namelist /mesh/ dim,nc1,nc2,nc3,ejecta_type,Mni0,Mfe0,Mime0,&
                      ekin,vej,vejmax,mesh_type,ismix,dmix,nmix,vejfacmax,mej
...
! Woosley & Kasen
          ...
          elseif(fac.gt.mfe0 .and. fac.le.(mfe0+mni0)) then
            atoms(indiso(28,56),i)=1.0d0     ! nickel
...
!           atoms(indiso(8,16),i)=0.5d0      ! O
      ...
      if (ejecta_type.eq.10) then !! read Nomoto's W7
      filename='~/yoni/profiles_from_kasen/w7_sphere_100.mod'
      open (unit=10,file=filename)

      read(10,*) fac
      read(10,*) fac
      do i=1,100
      read(10,*) v1(i+1),mass(i),atoms(indiso(28,56),i),atoms(indiso(2,4),i),atoms(indiso(4,8),i),&
      atoms(indiso(6,12),i),atoms(indiso(8,16),i),atoms(indiso(10,20),i),atoms(indiso(11,22),i),&
      atoms(indiso(12,24),i),atoms(indiso(14,28),i),atoms(indiso(16,32),i),atoms(indiso(18,36),i),&
      atoms(indiso(20,40),i),atoms(indiso(22,44),i),atoms(indiso(24,48),i),atoms(indiso(26,54),i),&
      atoms(indiso(27,54),i),atoms(indiso(28,54),i)
      v1(i+1)=v1(i+1)*1d5   !! convert from km/s
      vol=4.0d0*pi/3.0d0*(v1(i+1)**3.0d0-v1(i)**3.0d0)
      mass(ind(i,1,1))=mass(i)*vol*(20.0d0*day)**3.0d0
      rhov(ind(i,1,1))=1.0d0/vol

      atoms(indiso(28,54),i)=atoms(indiso(28,54),i)-atoms(indiso(28,56),i)
      atoms(indiso(26,54),i)=atoms(indiso(26,54),i)/2.0d0
      atoms(indiso(27,54),i)=atoms(indiso(27,54),i)/2.0d0
      atoms(indiso(28,54),i)=atoms(indiso(28,54),i)/2.0d0
      atoms(indiso(26,58),i)=atoms(indiso(26,54),i)
      atoms(indiso(27,58),i)=atoms(indiso(27,54),i)
      atoms(indiso(28,58),i)=atoms(indiso(28,54),i)
     
      atoms(:,i)=atoms(:,i)/sum(atoms(:,i))
...
1004  format(I5,1000(1pe14.6))
      ...
      function random_location(i,j,k,t,rng)
      ...
      if(present(rng)) then
        ...
        if (dim.eq.1) then
          ...
          random_location=v*random_unit_vec1(3,rng)

        elseif (dim.eq.2) then

          v=(v1(i)**2.0d0*z(1)+v1(i+1)**2.0d0*(1.0d0-z(1)))**(1.0d0/2.0d0)
          random_location(1:2)=v*random_unit_vec1(2,rng)
...
!       i2=i2-1
        ...
        mixfrac(i1)=1
        mixfrac(i2)=(mix2-summass(i2-1))/mass(i2)
